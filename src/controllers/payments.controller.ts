...
export class PaymentsController {
  ...
  static async createOrder(req: Request, res: Response) {
    try {
      ...
      let associatedPlanId: string | null = null;
      ...
      const razorpayOrder = await RazorpayService.createOrder(payableAmountInInr, orderReceipt, orderNotes);
  ...
  static async verifyPayment(req: Request, res: Response) {
    try {
      ...
      const confirmedPayment: any = await prisma.$transaction(async (tx) => {
        let currentPayment: any = payment;
  ...
  static async handleWebhook(req: Request, res: Response) {
    try {
      ...
      const eventId = (req.headers['x-razorpay-event-id'] as string) || req.body.event_id || `evt_${Date.now()}_${Math.random()}`;
