...
export class CreditService {
  ...
  static async calculateFee(budgetMin: number, budgetMax?: number | null): Promise<number> {
    const settingFeePct = await prisma.systemSetting.findUnique({
      where: { key: 'application_fee_percentage' },
    });
    const feePct = settingFeePct ? parseFloat(settingFeePct.value) : config.businessRules.applicationFeePercentage;

    const settingNominal = await prisma.systemSetting.findFirst({
      where: { key: { in: ['credit_value', 'credit_nominal_value'] } },
    });
    const nominalValue = settingNominal ? parseFloat(settingNominal.value) : (config.businessRules.creditNominalValue || 50.0);

    const settingMin = await prisma.systemSetting.findUnique({
      where: { key: 'minimum_application_credits' },
    });
    const minCredits = settingMin ? parseInt(settingMin.value, 10) : config.businessRules.minimumApplicationCredits;

    const settingMax = await prisma.systemSetting.findUnique({
      where: { key: 'maximum_application_credits' },
    });
    const maxCredits = settingMax ? parseInt(settingMax.value, 10) : config.businessRules.maximumApplicationCredits;
    ...
    if (credits < minCredits) credits = minCredits;
  ...
  static async deductCreditsForApplication(
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      if (wallet.balance < creditsCost) {
        throw new Error(
          `Insufficient credits. Required: ${creditsCost}, Available: ${wallet.balance}. Please purchase a credit plan to apply.`
        );
      }
