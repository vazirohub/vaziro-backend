...
export interface UserAccountContext {
  ...
  recentJobsSummary?: string;
  recentRefundsSummary?: string;
  recentPaymentsSummary?: string;
}

export class AIAccountToolsService {
  /**
   ...
   */
  static async getAccountContextForAI(userId: string): Promise<string> {
    try {
      ...
      // 1. Identity & Profile
      parts.push(
        `User: ${user.firstName} ${user.lastName} | Status: ${user.status} | Roles: ${roleNames.join(', ')}`
      );
      ...
      if (isPro && user.professionalProfile?.creditWallet) {
        ...
        parts.push(
          `Credit Wallet Balance: ${wallet.balance} credits | Lifetime Purchased: ${wallet.lifetimePurchased} | Lifetime Spent: ${wallet.lifetimeSpent}`
        );
        ...
        if (wallet.transactions.length > 0) {
          const txList = wallet.transactions
            .map(
              (t: any) =>
                `• ${t.transactionType}: ${t.amount > 0 ? '+' : ''}${t.amount} credits (New Balance: ${t.balanceAfter}) on ${new Date(t.createdAt).toLocaleDateString('en-IN')}`
            )
            .join('; ');
        ...
        if (wallet.creditRefunds.length > 0) {
          const refundList = wallet.creditRefunds
            .map(
              (r: any) =>
                `• ${r.creditsRefunded} credits (${r.status}) - Reason: ${r.reason} on ${new Date(r.createdAt).toLocaleDateString('en-IN')}`
            )
            .join('; ');
      ...
      if (user.payments && user.payments.length > 0) {
        const payList = user.payments
          .map(
            (p: any) =>
              `• ₹${p.amount} (Payment: ${p.status}) on ${new Date(p.createdAt).toLocaleDateString('en-IN')}`
          )
          .join('; ');
   ...
   */
  static isAccountSpecificQuery(prompt: string): boolean {
