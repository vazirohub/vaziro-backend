...
export interface SendNotificationOptions {
  userId: string;
  type: 'REQUIREMENT' | 'QUOTATION' | 'HIRE' | 'JOB_STATUS' | 'PAYMENT' | 'DISPUTE' | 'SYSTEM';
  title: string;
  message: string;
  actionUrl?: string;
  email?: {
...
export interface IWhatsAppProvider {
  sendWhatsAppMessage(toPhone: string, template: string, params?: Record<string, string>): Promise<boolean>;
}

export class MockWhatsAppProvider implements IWhatsAppProvider {
  async sendWhatsAppMessage(toPhone: string, template: string, params?: Record<string, string>): Promise<boolean> {
...
<body>
   ...
   */
  static async sendWelcome(user: { id: string; email?: string | null; firstName: string; roles?: string[] }): Promise<void> {
