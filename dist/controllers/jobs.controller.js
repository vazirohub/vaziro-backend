...
class JobsController {
    ...
    static async hire(req, res) {
        try {
            ...
            const job = await prisma_1.prisma.$transaction(async (tx) => {
                ...
                await credit_service_1.CreditService.refundNonHiredApplicants(quotation.requirementId, quotation.professionalProfileId, tx);
     ...
     */
    static async confirmCompletion(req, res) {
        try {
            const { id } = req.params;
            const userId = req.user?.id;
            const job = await prisma_1.prisma.job.findUnique({
            ...
            if (job.workStatus !== 'WORK_COMPLETED' &&
    ...
    static async getMyJobs(req, res) {
        try {
            ...
            if (roles.includes('CUSTOMER')) {
                const customer = await prisma_1.prisma.customerProfile.findUnique({ where: { userId } });
                if (customer)
                    whereClause.customerId = customer.id;
            }
            else if (roles.includes('PROFESSIONAL')) {
                const prof = await prisma_1.prisma.professionalProfile.findUnique({ where: { userId } });
                if (prof)
                    whereClause.professionalProfileId = prof.id;
            }
