                ...
                if (razorpayOrderId) {
                    await prisma_1.prisma.$transaction(async (tx) => {
                        ...
                        if (purpose === 'PROFESSIONAL_PLAN' && (notes.planId || notes.professionalProfileId)) {
                            try {
                                await credit_service_1.CreditService.fulfillPlanPurchase(notes.professionalProfileId, notes.planId, {
                                    paymentId: payment.id,
                                    razorpayOrderId,
                                    razorpayPaymentId,
        ...
        try {
            ...
            const result = await prisma_1.prisma.$transaction(async (tx) => {
                ...
                if (linkedAccount && linkedAccount.razorpayAccountId) {
                    try {
                        const transfer = await razorpay_service_1.RazorpayService.createTransfer(paymentRecord?.razorpayPaymentId || paymentRecord?.id || '', linkedAccount.razorpayAccountId, netPayout, { jobId: job.id, professionalProfileId: job.professionalProfileId });
