...
class CreditService {
     ...
     */
    static calculateFeePure(budgetMin, budgetMax) {
        const effectiveBudget = budgetMax && budgetMax > budgetMin ? budgetMax : budgetMin;
        if (!effectiveBudget || effectiveBudget <= 0 || isNaN(effectiveBudget)) {
            return 1;
        }
    ...
    static async getOrCreateWallet(professionalProfileId) {
        ...
        if (!wallet) {
            ...
            const welcomeBatch = await prisma_1.prisma.creditBatch.create({
                data: {
                    professionalProfileId,
            ...
            await prisma_1.prisma.creditLedger.create({
                data: {
                    professionalProfileId,
    ...
    static async getDetailedWallet(professionalProfileId) {
        ...
        const [activeBatches, refundPendingBatches, refundedApps, latestLedger, latestPlanPurchase] = await Promise.all([
            ...
            prisma_1.prisma.planPurchase.findFirst({
                where: {
                    professionalProfileId,
                    status: 'COMPLETED',
                },
     ...
     */
    static async deductCreditsForApplication(professionalProfileId, requirementId, creditsCost, customTx) {
        const executeDeduction = async (tx) => {
            ...
            if (wallet.balance < creditsCost) {
                throw new Error(`Insufficient credits. This requirement requires ${creditsCost} credits, but your available balance is ${wallet.balance}. Please get a Professional Plan to apply.`);
            }
        ...
        return await prisma_1.prisma.$transaction(executeDeduction);
    }
    /**
     ...
     */
    static async fulfillPlanPurchase(professionalProfileId, planId, paymentDetails = {}) {
     ...
     */
    static async processExpiredBatches(cutoffDate) {
        const targetDate = cutoffDate || new Date();
        const expiredBatches = await prisma_1.prisma.creditBatch.findMany({
            where: {
                status: 'ACTIVE',
        ...
        for (const batch of expiredBatches) {
            const batchResult = await prisma_1.prisma.$transaction(async (tx) => {
     ...
     */
    static async refundCreditsForCancellation(professionalProfileId, requirementId, creditsToRefund, reason) {
        return await prisma_1.prisma.$transaction(async (tx) => {
            const wallet = await tx.creditWallet.findUnique({
                where: { professionalProfileId },
            });
    ...
    static async refundCredits(professionalProfileId, creditsToRefund, requirementId, reason) {
        return await this.refundCreditsForCancellation(professionalProfileId, requirementId, creditsToRefund, reason);
    }
    /**
